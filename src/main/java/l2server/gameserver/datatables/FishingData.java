...
import l2server.gameserver.model.fishing.FishingBait;
import l2server.gameserver.model.fishing.FishingCatch;
import l2server.gameserver.model.fishing.FishingRod;
						...
						case "baits":
						{
							for (Node bait = listItem.getFirstChild(); bait != null; bait = bait.getNextSibling())
							{
								if ("bait".equalsIgnoreCase(bait.getNodeName()))
								{
									...
									final byte maxPlayerLevel = parseByte(attrs, "maxPlayerLevel", Config.MAX_LEVEL);
