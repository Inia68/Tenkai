...
import l2server.gameserver.model.event.TimerExecutor;
...
public class TimerHolder<T> implements Runnable
{
    ...
    public TimerHolder(T event, StatSet params, long time, L2Npc npc, L2PcInstance player, boolean isRepeating, IEventTimerEvent<T> eventScript, IEventTimerCancel<T> cancelScript, TimerExecutor<T> postExecutor)
    {
        ...
        _task = isRepeating ? ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(this, _time, _time) : ThreadPoolManager.getInstance().scheduleGeneral(this, _time);
